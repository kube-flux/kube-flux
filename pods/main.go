...
const HostIp = "34.67.11.84"
const MasterUrl = "https://" + HostIp
...
func authenticate(filePath string) *kubernetes.Clientset {
...
// listPods lists the number of pods in the cluster.
...
func changeReplica(clientSet *kubernetes.Clientset) {
	...
	*(nginx.Spec.Replicas) = newReplica
	_, _ = clientSet.AppsV1().Deployments(apiv1.NamespaceDefault).Update(context.TODO(), &nginx, metav1.UpdateOptions{})
...
func main() {
	...
	clientSet := authenticate(filePath) //Authenticates with the GCP cluster
	listPods(clientSet)                 //Lists the pods in the cluster
