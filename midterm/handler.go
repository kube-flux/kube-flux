...
var currentStatus Status = Green
...
func NewPolicyHandler(filePath string, clusterIP string) (*policyHandler, error) {
	...
	log.Println("printing clientSet: ", &clientSet)
...
func (handler *policyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		err := handler.db.View(func(tx *bolt.Tx) error {
			...
			log.Println("func", "ServeHTTP", "Response written", policy.Status)
	...
	if r.Method == "PUT" {
		...
		err := handler.db.Update(func(tx *bolt.Tx) error {
			...
			//Change number of replicaSets
			if currentStatus == policy.Status {
				log.Println("Same status as before: doing nothing...")
			} else {
				log.Println("Changing replicaSets...")
				changeReplica(clientSet, policy.Status)
