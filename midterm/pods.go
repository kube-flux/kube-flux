...
func changeReplica(clientSet *kubernetes.Clientset, signal Status) {
	namespace := apiv1.NamespaceDefault
	...
	var newReplica int32 = *(nginx.Spec.Replicas)

	//change number of replicaSets depending on signal
	switch signal {
	case Green:
		newReplica = 4
	case Yellow:
